...
;              1.1   04.26.2020  HOR     Changed to add $ and , for Totals    ;
;                                                                             ;
;=============================================================================;
                   INCLUDE           NCOMMON.TXT
                   INCLUDE           WORKVAR.INC
...
DelCheckFlag        form               1
;
; Let's see if we're going to create a duplicate check nunber
;
                   call              CheckDuplicateChecks
                   return            if (ErrorFlag)
...
PrintCheckLine
              ...
              edit              CheckRun.CheckAmount,DCheckAmount,mask=mask,ALIGN=1
              edit              CheckRun.GrossAmount,DGrossCheckAmount,mask=mask,ALIGN=1
...
.
              PrtPage           P;*font=DetailFont:
                                ...
                                *alignment=*decimal:
...HR 2020.04.26                                *p=725:(CheckStub + 080),CheckRun.CheckAmount:
                                *p=725:(CheckStub + 080),DCheckAmount:     //        CheckRun.CheckAmount:
                                *alignment=*left
...
PrintCheckLine2
              PrtPage           P;*font=DetailFont:
                                ...
                                *alignment=*decimal:
                                *p=445:(StubTotalLine1 + 210),DGrossCheckAmount:    //CheckRun.GrossAmount:
                                *p=595:(StubTotalLine1 + 210),CheckRun.DiscAmt:
                                *p=755:(StubTotalLine1 + 210),DCheckAmount:            //CheckRun.CheckAmount:
                                *p=445:(StubTotalLine2 + 240),DGrossCheckAmount:    //CheckRun.GrossAmount:
                                *p=595:(StubTotalLine2 + 240),CheckRun.DiscAmt:
...HR 2020.04.26                                *p=755:(StubTotalLine2 + 240),CheckRun.CheckAmount:
                                *p=755:(StubTotalLine2 + 240),DCheckAmount:       //CheckRun.CheckAmount:
                                *alignment=*left
...
DNetAmount         dim               17
DGrossAmount         dim               17
              edit              CheckRunDetail.NetAmount,DNetAmount,mask=mask,ALIGN=1
              edit              CheckRunDetail.GrossAmount,DGrossAmount,mask=mask,ALIGN=1
              ...
              PrtPage           P;*alignment=*left:
                                ...
                                *alignment=*decimal:
                                *p=445:(StubLine1 - 20),DGrossAmount:                  //CheckRunDetail.GrossAmount:
                                *p=595:(StubLine1 - 20),CheckRunDetail.DiscAmt:
                                *p=755:(StubLine1 - 20),DNetAmount:                    //CheckRunDetail.NetAmount:
                                *p=445:(StubLine2 - 20),DGrossAmount:                  //CheckRunDetail.GrossAmount:
                                *p=595:(StubLine2 - 20),CheckRunDetail.DiscAmt:
                                *p=755:(StubLine2 - 20),DNetAmount                     //CheckRunDetail.NetAmount
...
.                                *p=750:(CheckStub + 070),"VOID VOID VOID";
              ...
              RETURN
;==========================================================================================================
CheckDuplicateChecks
TestCheckNumber    form              10

                   clear             ErrorFlag
                   GETPROP           ECheckNumber,value=TestCheckNumber
                   ...
                   loop
                     call              KSCheckRun
                   until             (ReturnFL = 1)
                   continue          if (CheckRun.Printed = 1)
                     packkey           CheckKY from $Entity,CheckRun.BankCode,TestCheckNumber
                     call              RDCheck
                     if                (ReturnFl = 0)
                       pack              AlertString from "There will be a duplicate check created during this Check Run (Check # ",TestCheckNumber:
                                                          ").  Please use a different starting Check Number"
                       alert             note,AlertString,result,"POTENTIAL DUPLICATE CHECK nO."
                       set               ErrorFlag
                       break
                     endif
                     incr              TestCheckNumber
                   repeat
                   return
.=============================================================================
ExitProgram
