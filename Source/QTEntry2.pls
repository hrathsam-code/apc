...
MAIN                PLFORM             QTEntry.PLF
DataMenu            PLFORM             DataMenu.PLF
ShipToF             plform             QTEntry3.plf
...
.
                     ...
                     if                (LastEditRow = 0)               //We have no Kit Components
                       ...
                       CALL              GetOrderHeader
                       transaction       Start,SequenceFLST,QTHeaderFLST,QTDetailsFLST,ARTrnFLST,CustFLST,QTNotesFLST

                       GetNextSeq        POH
                       move              Sequence.SeqNo,QTHeader.SeqMajor
                       move              Sequence.SeqNo,QTHeader.Reference
                       CALL              WRTQTHeader
;
...
Dim20               dim                20
                   ...
                   if                (result =-1)
                     beep
                     alert              note,"A valid ShipTo must be selected",result,"INVALID SHIP TO"
                     move               "1",ValidFlag
                     setfocus           CBShipTo
                     return
                   endif
                   ...
                   for                 X from "1" to (LastEditRow - 1) using "1"
                     ...
                     getprop            NPrice(X),value=OrderPrice
                     MOVE               QTDetails.UnitPrice,ExtendedPrice
...
GetOrderHeader
                    ...
                    clear              QTHeader
                    call               GetMain
                    move               $Entity,QTHeader.Entity
                    CBTerms.GetCurSel  giving result
                    move               TermCodeArray(result),QTHeader.TermCode
                    move               TermCodeArray(result),ARTRMKY
                    call               RDARTrm

                    getprop            ESalesRep,text=QTHeader.SalesRep
                    clock              TimeStamp,QTHeader.TransDate
                    getprop            CBCompleted,value=QTHeader.Completed
                    clock              TimeStamp,QTHeader.BillDate
                    getprop            DTOrderDate,text=QTHeader.ShipDate
                    getprop            NTotals,value=QTHeader.InvoiceAmt
...                    move               QTHeader.InvoiceAmt,QTHeader.TotalAmt
                    getprop            NFreight,value=QTHeader.Freight
                    subtract           "1",LastEditRow,QTHeader.Lines
                    move               "N",QTHeader.Printed
                    move               "I",QTHeader.InvCredit
                    %endif
...
GETMAIN
                     %if               $EntryProgram = 2
                     getprop           ECustomerID,text=QTHeader.BillCust
                     move              QTHeader.BillCust,QTHeader.ShipToCust

                     getprop           DTOrderDate,text=QTHeader.OrderDate
..                     getprop           DTShipDate,Checked=EndDateFlag


                    CBShipmentCode.GetCurSel giving result
                    CBShipmentCode.GetText Giving QTHeader.ShipVia using *Index=result

                    getprop            STPONo,text=QTHeader.CustomerPO
                    getprop            CBCompleted,value=QTHeader.Completed
                    getprop            EWeight,value=QTHeader.Weight
                    getprop            ECartons,value=QTHeader.Cartons

                    getprop            ECustName,text=QTHeader.BillToName
                    getprop            ECustAddr1,text=QTHeader.BillToAddress1
                    getprop            ECustAddr2,text=QTHeader.BillToAddress2
                    getprop            ECustAddr3,text=QTHeader.BillToAddress3

                    getprop            ECustCity,text=QTHeader.BillToCity
                    getprop            ECustSt,text=QTHeader.BillToSt
                    getprop            ECustZip,text=QTHeader.BillToZip
                    getprop            ECustCountry,text=QTHeader.BillToCountry
;
; Main Ship-To information
;
                    getprop            STShipName,text=QTHeader.ShipToName

                    getprop            STShipAddress1,text=QTHeader.ShipToAddress1
                    getprop            STShipAddress2,text=QTHeader.ShipToAddress2
                    getprop            STShipAddress3,text=QTHeader.ShipToAddress3

                    getprop            STShipCity,text=QTHeader.ShipToCity
                    getprop            STShipSt,text=QTHeader.ShipToSt
                    getprop            STShipZip,text=QTHeader.ShipToZip
                    getprop            STShipCountry,text=QTHeader.ShipToCountry
...
..HR 2020.04.14                    clear              OrderDetailArray
                    ...
                    chop               QTDetails.PartNumber,QTDetails.PartNumber
                    continue           if (QTDetails.PartNumber = "")                              //HR 2019.7.22

                    switch             Status

                    case               1                           //Changed
                      packkey          QTDetailsKY from QTHeader.Reference,QTDetails.LineNo
                      call             TSTQTDetails
                      if               (ReturnFl = 1)
                        transaction      Rollback
                        alert            stop,"Error Updating Kit Detail Records",result
...
......                      call               RDParts
                      ...
                      move               QTHeader.ShipToCust,QTDetails.ShipToCust

                      move               QTHeader.Reference,QTDetails.Reference
                      move               QTHeader.SeqMajor,QTDetails.SeqMajor
                      move               X,QTDetails.SeqMinor

                      move               QTHeader.Entity,QTDetails.Entity
                      move               QTHeader.EntryDate,QTDetails.EntryDate
                      move               QTHeader.TransDate,QTDetails.TransDate
                      move               QTHeader.BillDate,QTDetails.BillDate
                      move               QTHeader.ShipDate,QTDetails.ShipDate
                      move               QTHeader.BillCust,QTDetails.BillCust
                      move               QTHeader.ShipToCust,QTDetails.ShipToCust
                      getprop            NExtended(X),value=QTDetails.ExtendedAmt
                      move               QTDetails.InvoiceAmt,QTDetails.TaxableAmt
                      move               QTDetails.InvoiceAmt,QTDetails.SalesAmt

....                      getprop            EProductGroup(X),text=QTDetails.Size
                      getprop            NQuantityOrdered(X),value=QTDetails.OrderQty
                      getprop            NQuantityShipped(X),value=QTDetails.ShipQty
                      getprop            NQuantityBackOrder(X),value=QTDetails.BackOrderQty

                      getprop            NPrice(X),value=QTDetails.UnitPrice
                      move               X,QTDetails.LineNo
                      getprop            EPartNumber(X),text=QTDetails.PartNumber
                      getprop            EUOM(X),text=QTDetails.UOM
                      clock              TimeStamp,QTDetails.TransDate
...                      move               Parts.Description1,QTDetails.Description1
...                      move               Parts.Description2,QTDetails.Description2
...                      move               Parts.TaxCode,QTDetails.TaxCode
                      clear              QTDetails.ReplacementPartNumber
                      getprop            Nextended(X),value=QTDetails.ExtendedAmt
...                      move               Parts.AllowFractional,QTDetails.AllowFractional
...                      move               Parts.SerialNumbered,QTDetails.SerialNumbered
...                      move               Parts.WarrantyIncluded,QTDetails.WarrantyIncluded
...                      move               Parts.WarrantyTime,QTDetails.WarrantyTime
...                      move               Parts.ServiceLife,QTDetails.ServiceLife
...                      move               Parts.Cost,QTDetails.Cost
...                      move               Parts.Size,QTDetails.Size
...                      move               Parts.LotNumber,QTDetails.LotNumber
...                      move               Parts.ItemType,QTDetails.ItemType
...
;
                      ...
                      getprop            NQuantityOrdered(LastEditRow),value=QTDetails.ORderQty

                      if               (PartsKY = "" and QTDetails.OrderQty != 0 )
                        beep
...
;
                    ...
                    if                 (ReturnFl = 1)
                      beep
                      clearevents
                      alert              stop,"Invalid Cust Account Number",result,"INVALID ACCOUNT"
                      setfocus           ECustomerID
                      return
                    else
...
.                    return
                     ...
                     debug
                     search            Cust.TermCode in TermCodeArray(0) from 99 to CBResult
;
; HR 20200403
;
                     if                (equal)
...
; Main Ship-To information
;
..HR 2020.04.15                    setprop            STShipName,text=Cust.Name

..HR 2020.04.15                    setprop            STShipAddress1,text=Cust.Addr1
..HR 2020.04.15                    setprop            STShipAddress2,text=Cust.Addr2
..HR 2020.04.15                    setprop            STShipAddress3,text=Cust.Addr3

..HR 2020.04.15                    setprop            STShipCity,text=Cust.City
..HR 2020.04.15                    setprop            STShipSt,text=Cust.St
..HR 2020.04.15                    setprop            STShipZip,text=Cust.Zip


....                    setprop            EShipToCustAddress2,text=Cust.Addr2
....                    setprop            EShipToCustAddress3,text=Cust.Addr3
...
OnClickShipTo
                    ...
                    if                 (result != 0)
                      ...
                      setprop            STShipAddress1,text=Cust.Addr1,static=1,BGColor=$BtnFace
                      setprop            STShipAddress2,text=Cust.Addr2,static=1,BGColor=$BtnFace
                      setprop            STShipAddress3,text=Cust.Addr3,static=1,BGColor=$BtnFace

                      setprop            STShipCity,text=Cust.City,static=1,BGColor=$BtnFace
                      setprop            STShipSt,text=Cust.St,static=1,BGColor=$BtnFace
                      setprop            STShipZip,text=Cust.Zip,static=1,BGColor=$BtnFace

                      if                 (Cust.Country = "   ")
                        move               "USA",ShipTo.Country
                      endif

                      setprop            STShipCountry,text=Cust.Country,static=1,BGColor=$BtnFace
                    else
...
..                SETITEM           RPrintViaPrinter,0,1      //HR 4/20/2005  Select the Printer now
                   ...
                   DEBUG
                PACKKEY           QTHeaderKY3 FROM $Entity,"N"
                PACKKEY           QTHeaderKY2 FROM $Entity,PrintFromDate

                CALL              RDQTHeader3 if (PrintType = "P")
                CALL              RDQTHeader2 if (PrintType = "R")
                MOVE              "Y",FirstPrint
                MOVE              "0",QTHeader.Reference
                MOVE              "0",SortedCounter
                CLEAR             SortedCust
                CLEAR             SortedInvoice

              LOOP
                CALL              KSQTHeader3 if (PrintType = "P")
                CALL              KSQTHeader2 if (PrintType = "R")
              UNTIL             ((QTHeader.Printed = "Y" or ReturnFL = 1) and PrintType = "P")
              UNTIL             ((QTHeader.TransDate > PrintToDate or ReturnFL = 1) and PrintType = "R")
..HR 2020.04.15                CONTINUE          IF (PrtInvType = 2 and QTHeader.InvCredit != "C")    //Ignore everything except Credit Memos
..HR 2020.04.15                CONTINUE          IF (PrtInvType = 3 and QTHeader.InvCredit != "D")    //Ignore everything except Debit Memos
..HR 2020.04.15                CONTINUE          IF (PrtInvType = 4 and QTHeader.InvCredit != "I")    //Ignore everything except Debit Memos
                CONTINUE          IF ((QTHeader.Reference < PrintFromInvoice or QTHeader.Reference > PrintToInvoice) and PrintType = "R")
                CONTINUE          IF (PrintType = "R" and QTHeader.Printed != "Y")
.
...
.
                PACKKEY           CustKY FROM QTHeader.BillCust
                CALL              RDCust
                CONTINUE          IF (ReturnFL = 1)
                ADD               "1",SortedCounter
                MOVE              Cust.Name,SortedCust(SortedCounter)
                MOVE              QTHeader.Reference,SortedInvoice(SortedCounter)
                IF                (PrintType = "P")                                      //HR 2019.7.9
                  move              QTHeader.Reference,InvoicesToReset(SortedCounter)    //Moved Up from below
                endif
              REPEAT
...
;
              FOR               IndexCounter FROM "1" TO SortedCounter USING "1"
                ...
                PACKKEY           QTHeaderKY FROM $Entity,SortedInvoice(IndexCounter)
                CALL              RDQTHeader
                IF                (ReturnFL = 1)
                ...
                MOVE              QTHeader.TermCode,ARTRMKY
                CALL              RDARTRM
                MOVE              QTHeader.BillCust,CustKY
                CALL              RDCust
...
; Moed from above  ^^^^^^^
                  ...
                  CONTINUE          IF (QTHeader.InvCredit = "C" and CopyNumber != 3)        //HR 5/6/2005 Only print 3rd copy for Credits
                  CONTINUE          IF (QTHeader.InvCredit = "D" and CopyNumber != 3)        //HR 4/9/2009 Only print 3rd copy for Credits

...                  CONTINUE          if (Cust.InvMethod = 4 and CopyNumber != 3)          //Do Not PRINT Option is selected  HR 3/8/2010

                  PACK              DocumentType from "Invoice :",QTHeader.Reference
;
...
..HR 2019.7.9                    MOVE              QTHeader.Reference,InvoicesToReset(ResetNumber)
..HR 2019.7.9                  ENDIF

                  MOVE              "860",FormLineNo
                  CALL              PrintQTHeader
                  PACKKEY           QTDetailsKY3 FROM QTHeader.Entity,QTHeader.SeqMajor
                  CALL              RDQTDetails3
                  LOOP
                    CALL              KSQTDetails3
                  UNTIL             (QTHeader.SeqMajor != QTDetails.SeqMajor or ReturnFL = 1)
...
.
                  CALC              InvSubtotal = QTHeader.InvoiceAmt - QTHeader.Freight
                  ...
                  PRTPAGE                   P;*Alignment=*Decimal:
                                            ...
                                            *P=1960:2380:                       //,*Alignment=*Decimal:
                                            QTHeader.Freight:
                                            *P=1960:2450,QTHeader.InvoiceAmt:
                                            *Alignment=*Left:
                                            *boldoff
                 endif
...
.
              ...
              IF                (PrintType = "P")
                FOR               X FROM "1" TO ResetNumber USING "1"
                  PACKKEY           QTHeaderKY FROM $Entity,InvoicesToReset(X)
                  CALL              RDQTHeader
                  CONTINUE          IF (ReturnFL = 1)                            //Record not found
                  CONTINUE          IF (QTHeader.Printed = "Y")                   //Already printed
                  MOVE              "Y",QTHeader.Printed
                  CALL              UPDQTHeader                                   //Update the Invoice Header record now to 'P'rinted
                REPEAT
              ENDIF
...
..HR 2020.04.14                    clear              OrderDetailArray
                    for                X from "1" to (LastEditRow - 1) using "1"
                      call               MoveEntryFieldsToInvoice
                      chop               QTDetails.PartNumber,QTDetails.PartNumber                  //HR 2019.8.16
                      debug
                      continue           if (QTDetails.PartNumber = "")                              //HR 2019.7.22
...
.
                   PACKKEY           QTHeaderKY FROM $Entity,Document
                   CALL              RDQTHeader
...
WriteAdditionalLines
                    ...
                    move               QTDetails.Entity,QTNotes.Entity
                    move               QTDetails.SeqMajor,QTNotes.SeqMajor
                    move               QTDetails.SeqMinor,QTNotes.SeqMinor
                    move               EAdditionalLines(X),QTNotes.Comments
                    if                 (PreviewFlag = 1)
                    else
                      call               WrtQTNotes
                    endif
...
AdditionalLineCount form               3
                    ...
                    if                 (PreviewFlag = 1)
                      ...
                      packkey            QTNotesKY from $Entity,QTDetails.SeqMajor,QTDetails.SeqMinor
                      call               RDQTNotes
...
Harry5
                    ...
                    explode            QTNotes.Comments,AlertReturn,PrtAdditionalLines
                    loop
...
PrintDetailLine
              PRTPAGE           P;*FONT="HELVETICA(9)":                                             //HR 2019.7.22
                                ...
                                *OverLayOn:
                                *boldon:
                                *V=FormLineNo,*H=190,QTDetails.OrderQty:
..HR 2020.04.14                                *H=390,QTDetails.ShipQty:
..HR 2020.04.14                                *H=590,QTDetails.BackOrderQty:
                                *alignment=*left:
                                *BoldOn:
..HR 2020.04.14                                *H=650,QTDetails.PartNumber:
                                *H=220,QTDetails.PartNumber:
                                *Boldoff:
                                *OverLayOff

PrtExtendedAmt      form               7.2

                    move               QTDetails.UnitPrice,PrtExtendedAmt
              ...
              PRTPAGE           P;*FONT="HELVETICA(9)":                                           //HR 2019.7.22
                                ...
                                *V=FormLineNo:
                                *boldon:
                                *H=1608,PrtExtendedAmt,*H=1772,QTDetails.UOM:
                                *H=1953,QTDetails.ExtendedAmt:
                                *boldoff:
                                *OverLayOff
...
;==========================================================================================================
PrintQTHeader
                    UNPACK            QTHeader.ShipDate into CC,YY,MM,DD
                    PACK              ShipDate FROM MM,slash,DD,slash,YY

                    UNPACK            QTHeader.BillDate into CC,YY,MM,DD
                    PACK              BillDate FROM MM,slash,DD,slash,YY

                    CHOP              QTHeader.ShipVia,QTHeader.ShipVia
                    ...
                    CHOP              QTHeader.ShipToCity          //HR 3/29/2005
...
;
                    IF                (Cust.POContact != "")
                      COUNT             CharCount,Cust.POContact
                      IF                (CharCount != 0)
                        MOVE              Cust.POContact,ContactName
                      ELSE
...
;
                    CHOP              QTHeader.ShipToAddress1
                    CHOP              QTHeader.ShipToAddress2
                    CHOP              QTHeader.ShipToAddress3

                    ResetVar          ShipToAddressLines
                    MOVE              "1",ShipToAddressLineCounter

                    IF                (QTHeader.ShipToAddress1 != "")
                      MOVE              QTHeader.ShipToAddress1,ShipToAddressLines(ShipToAddressLineCounter)
                      ADD               "1",ShipToAddressLineCounter
                    ENDIF

                    IF                (QTHeader.ShipToAddress2 != "")
                      MOVE              QTHeader.ShipToAddress2,ShipToAddressLines(ShipToAddressLineCounter)
                      ADD               "1",ShipToAddressLineCounter
                    ENDIF

                    IF                (QTHeader.ShipToAddress3 != "")
                      MOVE              QTHeader.ShipToAddress3,ShipToAddressLines(ShipToAddressLineCounter)
                      ADD               "1",ShipToAddressLineCounter
                    ENDIF

                    PACK              ShipToAddressLines(ShipToAddressLineCounter),QTHeader.ShipToCity,", ",QTHeader.ShipToSt,"  ",QTHeader.ShipToZip
                    ADD               "1",ShipToAddressLineCounter

                    IF                (QTHeader.ShipToCountry != "USA" and QTHeader.ShipToCountry != "   ")  //We're shipping to another Country
                      MOVE              QTHeader.ShipToCountry,CNTRYKY
                      CALL              RDCntry
                      IF                (ReturnFl = 1)
                    ...
                    PRTPAGE           P;*FONT="HELVETICA(9)":
                                      ...
                                      *FGCOLOR=*Black,*BGCOLOR=LightGrayRGB:
                                      *OVERLAYOFF:
                                      *V=725,*H=056,"QUANTITY":                         //,*H=645,"DESCRIPTION":
                                      *v=760:
...
..HR 202004.15                                      *P=020:360,"S",*H=020,*V=395,"O",*H=020,*V=430,"L":
..HR 202004.15                                      *P=020:465,"D",*H=020,*V=530,"T",*H=020,*V=565,"O":
...
..HR 20200317                                      *V=643,*H=20,"CUSTOMER ORDER NO.",*H=306,"DATE SHIPPED":
                                      *V=643,*H=20,"Cust ACCOUNT NO.",*H=413,"DATE ORDERED":
..HR 2020.04.15                                      *H=606,"WEIGHT",*H=756,"CARTONS":
..HR 20200317                                      *H=806,"SHIPPED VIA",*h=1306,"TERMS",*H=1706,"ORDER":
                                      *H=906,"SHIP VIA",*h=1306,"TERMS":                      //HR 2020.04.15 ,*H=1706,"ORDER":
                                      *v=685:
...
..HR 2020.04.15                                      *h=1910,"PARTIAL",*RECT=685:715:1870:1900:
                                      ...
                                      *FGCOLOR=*BLACK,*BGCOLOR=*WHITE:
                                      *FONT="HELVETICA(9)":
                                      *V=300,*H=220,"ACCT ## ",QTHeader.BillCust:
                                      ...
                                      *FONT="HELVETICA(9)":
..HR 2020.04.15                                      *V=300,*H=1440,"ACCT ## ",QTHeader.ShipToCust:
                                      *V=360,*H=1440,"APC COMPONENTS, INC.":           //QTHeader.ShipToName:
                                      *V=405,*H=1440,"75 TOLEDO STREET":               //ShipToAddressLines(1):
                                      *V=450,*H=1440,"FARMINGDALE, NY  11735":          //ShipToAddressLines(2):
...
..HR 2020.04.15                                      *P=1480:2610,"Unauthorized returns will not be accepted":
                                  ...
                                  *V=682,*H=20,QTHeader.CustomerPO:
                                  *H=427,ShipDate:
..HR 2020.04.15                                  *H=615,QTHeader.Weight:
..HR 2020.04.15                                  *H=770,QTHeader.Cartons:
                                  *H=910,QTHeader.ShipVia:
                                  *H=1310,ARTRM.Desc:
                                  *boldoff:
                                      *PL;
...
..HR 2020.04.15                    if                 (QTHeader.Completed = 1)
...
.                                      *P=920:2350,"P.O. Box 255":
                                      *P=860:2360,"Farmingdale, NY  11735":
                                      *P=800:2420,"accounting@apccomponents.com":
                                      *boldoff;
...
SortInvoicesByCust
              FOR                IndexCounter FROM "1" TO (SortedCounter - 1) USING "1"
                FOR                IndexCounter2 FROM "1" TO (SortedCounter - IndexCounter) USING "1"
                  IF                (SortedCust(IndexCounter2) > SortedCust(IndexCounter2 + 1))
                    MOVE              SortedCust(IndexCounter2 + 1),TempInvCust
                    MOVE              SortedCust(IndexCounter2),SortedCust(IndexCounter2 + 1)
                    MOVE              TempInvCust,SortedCust(IndexCounter2)
...
.
              ...
              IF                ( ReturnFL = 1 )
..HR 2019.11.11                pack                 dataline from ":",Document,":","  Key:",QTHeaderKY
..HR 2019.11.11                alert             note,DataLine,result
                ALERT             stop,"Invoice/Credit does not exist",result
                SETMODE           *MCURSOR=*Arrow
              ELSE
                MOVE              QTHeader.TransDate,PrintFromDate              // Set the lowest possible Date
                MOVE              QTHeader.TransDate,PrintToDate                // Set the highest possible Date
                CALL              PrintInvoices2                                //
              ENDIF
              RETURN
;==========================================================================================================
...
;==========================================================================================================
ShipVia
                   uppercase         QTHeader.ShipVia,QTHeader.ShipVia
                   scan              "UPS",QTHeader.ShipVia
                   if                (equal)
                     beep
...
;==========================================================================================================
LoadUPSRecord
                   move              QTHeader.ShipToName,UPSRecord.ShipToName
                   move              QTHeader.ShipToAddress1,UPSRecord.ShipToAddress1
                   move              QTHeader.ShipToAddress2,UPSRecord.ShipToAddress2
                   move              QTHeader.ShipToAddress3,UPSRecord.ShipToAddress3
                   move              QTHeader.ShipToCity,UPSRecord.ShipToCity
                   move              QTHeader.ShipToSt,UPSRecord.ShipToState
                   move              QTHeader.ShipToZip,UPSRecord.ShipToZip
                   move              QTHeader.ShipToCountry,UPSRecord.ShipToCountry
                   move              QTHeader.Cartons,UPSRecord.Cartons
                   move              QTHeader.Weight,UPSRecord.Weight
                   move              QTHeader.ShipVia,UPSRecord.ShipVia
                   move              Cust.ShippingAcct(2),UPSRecord.ShipToAcctNo
                   move              QTHeader.InvoiceAmt,UPSRecord.InvoiceAmount
                   return
;==========================================================================================================
