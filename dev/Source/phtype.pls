...
;                                                                             ;
;     DATE:    11/06/2003 AT 4:39AM                                           ;
;                                                                             ;
;  PURPOSE:    Maintenance program to update the Phone Types                  ;
;                                                                             ;
; REVISION:    VER     DATE     INIT       DETAILS                            ;
;                                                                             ;
;              1.0   11/06/2003   HOR     INITIAL VERSION                     ;
;                      TESTING                                                       ;
;=============================================================================;
.
           INCLUDE           WORKVAR.INC
...
.
           ...
           CLOCK             INI,EnvData
           IF                NOT OVER
             PACK              EnvData,EnvData,PhoneTypeHelp
...
PhoneTypeCOLL  COLLECTION
PhoneTypeEDT   COLLECTION
           LISTINS           PhoneTypeCOLL,EPhoneTypeDescription,CXExtension

           LISTINS           PhoneTypeEDT,EPhoneTypeDescription
...
INITSRCH
           PACK              SearchTitle,PhoneTypeTitle," Search Window"
..           SETPROP           LVSearch,*TitleText=SearchTitle

           CTADDCOL          "Phone Tye Code",50,"Description",160:

           CTLoadTable       PhoneType,Type,Description
           RETURN
.
...
NOFILE
           NORETURN
           ALERT             PLAIN,"Phone Type file does not exist. Do you wish to create it?",#RESULT
           IF                (#RESULT = 1)
             CALL              PREPPhoneType
             GOTO              OPENFILES
           ENDIF
...
.X .
.X PRTREPORT
.X PhoneTypeRPT   PLFORM            PhoneTypeRPT.PLF
.X .           WINHIDE
.X
.X            CLOCK             DATE,TODAY8
.X            CLOCK             TIME,TIME8
...
.X
.X            GETITEM           EFromRange,0,PhoneTypeKY
.X            GETITEM           EToRange,0,ToCode
.X
.X            IF                (ToCode < PhoneTypeKY)
.X            BEEP
...
.X            LOOP
.X              CALL              KSPhoneType
.X            UNTIL             (RETURNFL = 1 OR PhoneType.Code > ToCode)
.X              CALL              PRTLINE
.X            REPEAT
.X .
.X . Print Totals line
...
.X              CALL              PRTHEADER
.X            ENDIF
.X            CHOP              PhoneType.NETDAYS,PhoneType.NETDAYS
.X            CHOP              PhoneType.DISCDAYS,PhoneType.DISCDAYS
.X
.X            MOVE              PhoneType.NETDAYS,NUM1
.X            MOVE              PhoneType.DISCDAYS,NUM2
.X
.X            PRTPAGE           PRTFILE;*1,PhoneType.Code:
.X                                      *16,PhoneType.Desc:
.X                                      *Alignment=*Right:
.X                                      *38,Num1:
.X                                      *50,Num2:
.X                                      *Alignment=*Decimal:
.X                                      *61,PhoneType.DiscPerc:
.X                                      *Alignment=*Left
.X
.X            ADD       "2" TO LINE
.X            RETURN
...
MainValid
           IF                (Status = 0)
...             GETCOUNT          EPhoneTypeCODE
             MOVE               "1",CharCount
             IF                (CharCount > 0)
               GETITEM           EPhoneType,0,result
               GETITEM           EPhoneType,result,PhoneTypeKY
               CALL              RDPhoneType
               IF               (RETURNFL = 1)
                 PARAMTEXT        PhoneTypeTITLE,PhoneTypeKY,"",""
...
.
           SETPROP           PhoneTypeEDT,SELECTALL=$SelectAll
.
           CALL              MAINRESET
           RETURN
.=============================================================================
...
. to Non Read-Only
.
             ENABLEITEM        PhoneTypeEDT
             DISABLEITEM       Fill1
             %IFDEF            CBPhoneTypeActive
             ENABLEITEM        CBPhoneTypeActive
             %ENDIF

             SETPROP           PhoneTypeEDT,READONLY=0
             SETPROP           PhoneTypeEDT,BGCOLOR=$WINDOW
.
. We also need to set any ActiveX controls to the same properties
.
.             SETPROP           EPhoneTypeDiscPerc,*Enabled=1
.             SETPROP           EPhoneTypeDiscPerc,*BackColor=$Window
.
. Setup any DEFAULT values
.
.             SETITEM           EPhoneTypeDISCDAYS,0,"0"
.
...
. reached.
           IF                (Status = 1)                //'Save' button has been pressed
             ...
             IF                (ValidFlag = 0)           //Great..All fields ARE valid!!!
               ...
               CALL              UPDPhoneType                //Update the record
...
...           GETCOUNT          EPhoneType
           MOVE               "1",CharCount
           IF                (Charcount > 0)
             GETITEM           EPhoneType,0,result      //Read the Primary field ito the Key
             GETITEM           EPhoneType,result,PhoneTypeKY      //Read the Primary field ito the Key

             CALL              RDPhoneTypeLK               //Lock the record so that nobody uses it
.
...
.
             ENABLEITEM        PhoneTypeCOLL
             DISABLEITEM       Fill1
             %IFDEF            CBPhoneTypeActive
             ENABLEITEM        CBPhoneTypeActive
             %ENDIF
             SETPROP           PhoneTypeCOLL,READONLY=0
             SETPROP           PhoneTypeCOLL,BGCOLOR=$WINDOW
.
...
.
             ...
             DISABLEITEM       BMainNew
.
             SETFOCUS          EPhoneTypeDescription        //Set the cursor to the next field

             DISABLEITEM       EPhoneType                   //and Disable the Primary Code
           ENDIF
           RETURN
.=============================================================================
...
.=============================================================================
MainFind
           FindSearch        EPhoneType
           IF                (PassedVar = "Y")
.             GETITEM           EPhoneType,0,result
.             GETITEM           EPhoneType,result,PhoneTypeKY
             MOVE              $SearchKey,PhoneTypeKY
             CALL              RDPhoneType
.
...
.
             MOVE              "0",Status
           MOVE                 PhoneType.Type,result
           SETITEM              EPhoneType,0,result
             CALL              MainValid
           ENDIF
           RETURN
.=============================================================================
...
MainFirst
           CLEAR             PhoneTypeKY
           FILL              FirstASCII,PhoneTypeKY
           CALL              RDPhoneType
           IF                (RETURNFL = 1)  . We didn't find a 'Blank' record
             CALL              KSPhoneType         . Try the 'next' record
...
MainLast
           CLEAR             PhoneTypeKY
           FILL              LastASCII,PhoneTypeKY
           CALL              RDPhoneType
           IF                (RETURNFL = 1)  . We didn't find a 'Blank' record
             CALL              KPPhoneType         . Try the 'Previous' record
             IF                (RETURNFL = 1)  . There are no records in the file
               BEEP
               ALERT             STOP,"No records exist in the system...",RESULT:
                                      LastTitle
...
. Reset the fields to 'Blank' and DISABLE all of those fields as well
           DELETEITEM        PhoneTypeCOLL,0
           DISABLEITEM       PhoneTypeCOLL
           SETPROP           PhoneTypeCOLL,READONLY=1
           SETPROP           PhoneTypeCOLL,BGCOLOR=$BTNFACE
           ENABLEITEM        Fill1
.
...
.
.           SETPROP           EPhoneTypeDiscPerc,*Text="0"
.           SETPROP           EPhoneTypeDiscPerc,*Enabled=0
.           SETPROP           EPhoneTypeDiscPerc,*BackColor=$BTNFACE
.
. Setup the Primary field that is used for Entry purposes
.
           %IFDEF            CBPhoneTypeActive
           SETITEM           CBPhoneTypeActive,0,0
           DISABLEITEM       CBPhoneTypeActive
           %ENDIF

..         SETPROP           EPhoneType,READONLY=0
           SETPROP           EPhoneType,BGCOLOR=$WINDOW
           ENABLEITEM        EPhoneType
           SETFOCUS          EPhoneType
           RETURN
.=============================================================================
...
.
MainDelete
           PARAMTEXT        PhoneType.Type,PhoneTypeTitle,"",""
           BEEP
...
SETMAIN
           MOVE                 PhoneType.Type,result
           SETITEM           EPhoneType,0,result
           SETITEM           EPhoneTypeDescription,0,PhoneType.Description
           SETITEM           CXExtension,0,PhoneType.Use_Extension

           %IFDEF            CBPhoneTypeActive
           SETITEM           CBPhoneTypeActive,0,PhoneType.Active
           %ENDIF
...
GETMAIN
           GETITEM           EPhoneType,0,result
           GETITEM           EPhoneType,result,PhoneType.Type
           GETITEM           EPhoneTypeDescription,0,PhoneType.Description
           GETITEM           CXExtension,0,PhoneType.Use_Extension

           %IFDEF            CBPhoneTypeActive
           GETITEM           CBPhoneTypeActive,0,PhoneType.Active
           %ENDIF
           RETURN
.=============================================================================
