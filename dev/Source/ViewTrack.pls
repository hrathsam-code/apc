...
..                    open               Inputfile,"Track_SampleRequest.xml",READ
                    ...
                    close              InputFile
                    clock              TimeStamp,DateRangeTo
                    move               DateRangeTo,DateRangeFrom

                    call               ConvDateToDays using DateRangeFrom,TodayDays
                    subtract           "60",TodayDays
                    call               ConvDaysToDate using TodayDays,DateRangeFrom
                    packkey            InvoicesKY5 from $Entity,PurchaseOrder
                    call               RDInvoices5
                    call               KSInvoices5
                    chop               Invoices.CustomerPO,Invoices.CustomerPO

                    if                 (ReturnFl = 1 or Invoices.CustomerPO != PurchaseOrder)
                      alert              note,"Unable to find Invoice with this Purchase Order No.",result,"ERROR: UNABLE TO LOCATE"
                      return
                    endif
CountryCode2       dim               2
                   if                (Invoices.ShipToCountry = "   ")
                   move              "USA",Invoices.ShipToCountry
                   endif

                   move              Invoices.ShipToCountry,CountryCode2

                    chop               Invoices.ShipToZip,Invoices.ShipToZip
                    pack               InputStream from InputStream2:
                                                        "<Code>01</Code> ":
                                                        "<Value>",PurchaseOrder,"</Value>":
                                                        "</ReferenceNumber> ":
                                                        "<PickupDateRange>":
                                                        "<BeginDate>",DateRangeFrom,"</BeginDate>":
                                                        "<EndDate>",DateRangeTo,"</EndDate>":
                                                        "</PickupDateRange>":
..HR 2019.11.7                                                        "<DestinationCountryCode>",CountryCode2,"</DestinationCountryCode>":
..HR 2019.11.7                                                        "<DestinationPostalCode>",Invoices.ShipToZip,"</DestinationPostalCode>":
                                                        "</TrackRequest>"
...
SavePosition       form              9
                   ...
                   http               HostName,ResourcePath,*Flags=Flags,*Port=443,*HTTPResult=OutputResults:
                                                            *result=HttpOutput,*PostMessage=InputStream,*Trace=TraceFileName
//                                                          *FileName=OutputFileName //,*Trace=TraceFileName,
...
;
                   ...
                   prep              OutputFile,"\apc\source\upstrack.xml",exclusive
...
;
                   ...
                   HttpOutputXML.GetPosition giving TrackPosition
...
;
                   HttpOutputXML.FindNode giving result9 using *Filter="Label='Shipment'",*Position=TrackPosition  //0          //Parent Node
...
;
; Let's see if they've rescheduled the date
;
                   if                (EstDeliveryDate = "")          //Check to see if the package has been rescheduled
                     HttpOutputXML.FindNode giving result9 using *Filter="ParentLabel='RescheduledDeliveryDate'",*Position=PackagePosition
                     if                (equal)
                       HttpOutputXML.GetText giving EstDeliveryDate
                     endif

                   debug
                     HttpOutputXML.FindNode giving result9 using *Filter="ParentLabel='RescheduledDeliveryTime'",*Position=PackagePosition
                     if                (equal)
                       HttpOutputXML.GetText giving EstDeliveryTime
                     endif
                   endif
